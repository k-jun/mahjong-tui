      ...
      case "naki": {
        ...
        switch (params.set.type) {
          ...
          case PaiSetType.MINKAN: {
            candidate = this.canMinkan({
              pai: params.set.paiCall[0],
              fromWho: params.set.fromWho,
            });
            break;
          }
        ...
        const isEqual = (e: PaiSet): boolean => {
          ...
          return e.pais.map((e) => e.fmt).join(",") ===
            params.set!.pais.map((e) => e.fmt).join(",");
        };
