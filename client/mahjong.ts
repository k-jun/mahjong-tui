...
export class Mahjong {
  ...
  gameStart(pais: Pai[]): void {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 4; j++) {
        this.users[(j + this.kyoku) % 4].paiRest.push(
  ...
  updateStatus(): boolean {
    ...
    if (
      this.kyoku >= 8 && !this.isRenchan &&
      this.users.some((e) => e.point >= 30000)
    ) {
      isCompleted = true;
    }
    if (this.kyoku >= 12) {
    ...
    if (isCompleted) {
      ...
      // 終了時の供託はトップ取り
      const topIdx = this.users.reduce(
        (maxIdx, user, idx) =>
          user.point > this.users[maxIdx].point ? idx : maxIdx,
        0,
      );
      this.users[topIdx].point += this.kyotaku * 1000;
    }
    return isCompleted;
  }
  ...
  dahai(params: MahjongParams): [MahjongInput, MahjongParams] {
    ...
    if (this.paiRinshan.length === 0) {
      const isKan = (e: PaiSet) =>
        [PaiSetType.ANKAN, PaiSetType.MINKAN, PaiSetType.KAKAN].includes(
          e.type,
        );
      let cnt = 0;
  ...
  richiBefore(params: MahjongParams): [MahjongInput, MahjongParams] {
    ...
    return [MahjongInput.DAHAI, {
      usrId: user.id,
      dahai: { paiId: params.richi!.paiId },
    }];
  }
  ...
  naki(params: MahjongParams): [MahjongInput, MahjongParams] {
    ...
    const set = new PaiSet({
      type: convertor[params.naki!.type],
      paiRest,
      paiCall,
      fromWho,
    });
    ...
    if (!user.naki({ set })) {
      this.debug(
        `naki failed: ${user.id}, ${set.pais.map((e) => e.fmt).join(",")}`,
      );
      return [MahjongInput.BREAK, params];
    }

    if (user.id !== tacha.id) {
      tacha.paiCalled.push(
        tacha.paiKawa.splice(tacha.paiKawa.length - 1, 1)[0],
      );
    }
    this.turnMove({ user });
    ...
    if (
      ...
      switch (set.type) {
        ...
        case PaiSetType.KAKAN:
          ...
          this.actions = this.actionAfterKakan({
            from: user,
  ...
  actionAfterKakan(
    { from, pai }: { from: MahjongUser; pai: Pai },
  ): MahjongAction[] {
  ...
  actionAfterTsumo(
    ...
    if (
      ...
      for (const s of setKakan) {
        naki.push({
          type: "kakan",
          usrId: this.users[(userIdx + s.fromWho) % 4].id,
          pmyId: s.paiRest.map((e) => e.id),
          purId: s.paiCall.map((e) => e.id),
        });
      }
      actions.push({ user, type: MahjongActionType.KAKAN, options: { naki } });
    }
  ...
  calcPoint(
    { user, from, pai, isChankan: isChnkn }: {
      user: MahjongUser;
      from: MahjongUser;
      pai: Pai;
      isChankan: boolean;
    ...
    if (isTsumo) {
      ...
      pointMinus = this.agariMinusPointRon({
        user,
        from,
        tokuten: output,
        pao,
  ...
  findAction(
    ...
    if (input === MahjongInput.NAKI) {
      return this.actions.find((e) =>
        e.user.id === user.id && e.type === params.naki!.type
      );
    }
    if (input === MahjongInput.AGARI) {
      ...
      return this.actions.find((e) =>
        e.user.id === user.id &&
        e.type === (isTsumo ? MahjongActionType.TSUMO : MahjongActionType.RON)
      );
    }
    if (input === MahjongInput.OWARI) {
      return this.actions.find((e) =>
        e.user.id === user.id && e.type === MahjongActionType.OWARI
      );
    }
    ...
    return this.actions.find((e) =>
      e.user.id === user.id && e.type === convertMap[input]!
    );
  }
  ...
  validate(input: MahjongInput, params: MahjongParams): boolean {
    ...
    if (this.status !== "playing") {
      this.debug(`status not playing: ${this.status}, input ${input}`);
      return false;
    }
    ...
    outer: while (true) {
      switch (input) {
        ...
        case MahjongInput.NAKI: {
          ...
          const existActions = this.actions.filter((e) =>
            e.type !== MahjongActionType.DAHAI
          ).length !== 0;
